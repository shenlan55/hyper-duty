...
**修改内容**:
1. **原值班人员显示优化**：将原值班人员显示框从简单的div改为禁用的el-select组件，使其样式与其他下拉框保持一致
2. **动态获取当前登录用户信息**：实现了从JWT令牌中解码获取当前登录用户的中文名，确保原值班人员显示框显示真实的用户名，而不是静态文本
3. **错误处理优化**：增强了用户信息获取的错误处理，确保在获取用户信息失败时不会导致页面报错

**技术实现**:
- 修改了`frontend/src/views/duty/SwapRequest.vue`组件，使用el-select组件显示原值班人员信息
- 更新了`frontend/src/utils/auth.js`文件，添加了JWT令牌解码功能，用于获取用户的中文名
- 实现了computed属性和ref对象管理，确保用户信息的动态更新
- 优化了错误处理逻辑，确保在获取用户信息失败时返回默认值，避免页面报错

**解决的问题**:
- 原值班人员显示框样式与其他下拉框不一致的问题
- 原值班人员显示框显示静态文本"当前用户"，而不是真实用户名的问题
- 获取用户信息失败时页面报错的问题
