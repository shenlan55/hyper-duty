...
**修复记录**:
- **问题**: 排班模式管理中人数输入字段加载失败，显示"--"而不是默认值1
- **原因**: 数据类型转换问题，count字段在解析和更新过程中没有正确保持为数字类型
- **解决方案**:
  1. 在解析configJson时，确保count字段被转换为数字类型：`count: Number(shift.count) || 1`
  2. 在watch监听器中，添加类型检查和转换：`if (shift.count === undefined || shift.count === null || isNaN(shift.count)) { shift.count = 1 } else { shift.count = Number(shift.count) }`
  3. 在生成configJson时，确保count字段保持为数字类型：`count: Number(shift.count) || 1`
  4. 确保新添加的班次和天数的count字段也正确初始化为数字类型

**核心逻辑**:
- 排班模式管理：支持新增、编辑、查看、删除排班模式
- 排班模式编排界面：横头是天，可以拓展第一天、第二天、第三天，每个天下面一行可以选择班次配置里面启用状态的班次，每天班次也可以横向追加，再下一行可以对应班次设置班次人数
...
- **排班模式编辑界面**：支持配置基本信息（名称、编码、类型、描述等）、天数（可添加/删除）、班次（从启用的班次配置中选择，可添加/删除）、人数（为每个班次设置人数）
