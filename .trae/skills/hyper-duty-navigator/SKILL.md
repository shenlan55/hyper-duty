...
### 19. 前端显示员工姓名必须优先用 DTO 注入的 employeeName（2026-06-27 新增）

- **背景**：请假审批列表/弹框申请人"刘润泽"显示"未知"，但后端 API 实际返回了 `employeeName: "刘润泽"`。根因是前端 `LeaveApproval.vue` 列表/弹框模板直接 `{{ getEmployeeName(row.employeeId) }}`，而 `getEmployeeName` 只从 `scheduleEmployeeList` / `employeeList` / `userStore` 查，**查不到当前选中值班表外的申请人**。当前登录用户命中 `userStore` 兜底才没显示"未知"，所以"王国豪"正常但"刘润泽"异常
- **强制规范**：
  - ✅ **优先**：`{{ row.employeeName || getEmployeeName(row.employeeId) }}` —— 后端 DTO 注入的字段，跨表一致
  - ✅ **兜底**：`getEmployeeName(id)` 仅当 DTO 字段为空时使用
  - ❌ **禁止**：直接 `{{ getEmployeeName(row.employeeId) }}` 不回退 DTO 字段（**`LeaveApproval.vue` line 95/142 踩坑**）
- **为什么不能依赖 scheduleEmployeeList 反查？**
  - `getScheduleEmployeesWithDetails(scheduleId)` 只返回当前选中值班表的员工
  - 请假申请人的 `scheduleId` 可能等于、也可能不等于当前查询的 scheduleId
  - 即使 scheduleId 相同，申请人也不一定在排班员工列表里
- **新功能自检清单**：
  1. 后端是否在 `fillDTOList` 中注入了 `employeeName` 字段？（IPage<DTO> 应该都有）
  2. 前端模板是否优先用 `row.employeeName` / `currentRequest.employeeName`？
  3. `getEmployeeName` 是否只作为 DTO 字段缺失时的兜底？
  4. 审批 / 详情 / 弹框模板是否都用了同样的"DTO 优先 + 函数兜底"写法？
- **参考实现**：`LeaveApproval.vue` line 95（列表）+ line 142（审批弹框，已修）+ line 234（查看弹框，原本正确）
