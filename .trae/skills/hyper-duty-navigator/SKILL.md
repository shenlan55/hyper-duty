...
### 18. 部门查询必须过滤 status=1（启用部门），禁止查到禁用部门（2026-06-27 新增）
- **全系统铁律**：除 `system`（系统管理）模块外，**所有模块查询 sys_dept 都必须只返回启用部门**（status=1）
- **背景**：2026-06-27 修复 — 继 #17 人员查询后，用户发现"部门"也需要同样的过滤（"现在看下部门是不是也要和人一样"）；当前数据库 8 个部门全是 status=1，但表结构已支持禁用，需提前规范
- **强制规范**：

  | 场景 | 规范 | 原因 |
  |------|------|------|
  | 通用部门查询（`getAllDepts` / `getDeptTree`） | **必须**只查启用部门 | 选人/选部门场景不该出现已禁用部门 |
  | 部门统计（`DutyStatisticsServiceImpl.getDeptStatistics`） | **必须**过滤禁用部门 | 历史排班即使绑定了禁用部门，统计时也要排除 |
  | 系统管理-部门分页（`SysDeptServiceImpl.page(...)`） | ❌ **故意不过滤** | 管理员需要看到所有部门（含禁用）来管理 |
  | 系统管理-部门树（`Dept.vue`） | ❌ **故意不过滤** | 部门管理页面需要管理所有部门 |
  | 系统统计-部门总数（`SystemStatisticsController`） | ❌ **故意不过滤** | 统计数字要全量 |

- **统一入口（推荐使用，不要直接 `lambdaQuery().list()`）**：
  ```java
  // ✅ 正确：通过通用接口拿启用部门
  sysDeptService.getAllDepts();                                  // 全量启用
  sysDeptService.getDeptTree();                                  // 部门树（已过滤）
  sysDeptService.lambdaQuery()
  ...
  ```
- **缓存一致性**：部门状态变更时，SysDeptServiceImpl.clearAllDeptCache() 已清掉 `dept::allDepts` + `dept::deptTree` ✅（2026-06-27 验证）
- **不联动 employee 查询（A 方案）**：员工被禁过滤自己（status=1），员工所在部门被禁**不过滤**（避免引入 JOIN 慢查询 + 避免管理员误禁部门后无法通过 dept_id 找回员工）。详见 2026-06-27 用户确认的 A 方案
- **新功能自检清单**（提交前必过）：
  1. 这次新增的部门查询接口，调用了 `getAllDepts` / `getDeptTree` / `lambdaQuery().list()` 中的哪一个？
  2. 选人/统计场景 → 是否过滤了 `status=1`？
  3. 部门更新时是否清掉了所有部门 cache？
  4. 前端 PersonSelector / 部门下拉框是否只拉取了启用部门？
- **参考实现**：`SysDeptServiceImpl.getAllDepts` / `SysDeptServiceImpl.getDeptTree` / `DutyStatisticsServiceImpl.getDeptStatistics`
