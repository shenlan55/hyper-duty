...
## 🏛️ 业务字典强制规范（2026-06-10 新增，强制！）

**业务枚举（任务状态/班次/审批/是否/性别/启用禁用/请假类型/排班模式/定时任务状态 等）必须统一走 `sys_dict_type + sys_dict_data` 字典表，禁止硬编码。**

详细规范、SQL 脚本、前端用法、新增流程、踩坑经验 → 查 `docs/业务字典使用手册.md`（专题入口）

通用指引、问题排查 → 查 `.trae/skills/hyper-duty-navigator/SKILL.md`（避坑 #7、#8）
