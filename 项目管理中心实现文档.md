...
**核心逻辑**：
1. 如果当前页是空的，直接添加根任务及其所有子任务
2. 如果当前页已有任务：
   - 先尝试添加下一个根任务及其所有子任务
   - 添加后检查是否超过 `pageSize`
   - 如果超过：当前页还是保留这个根任务，保存当前页，然后下一个根任务直接新开一页
   - 如果没超过：继续在当前页添加下一个根任务
...
**分页计算过程**：
1. 当前页空，添加根任务A（8条），当前页任务数=8
2. 尝试添加根任务B（4条），添加后总数=8+4=12 > 10
3. 超过了！但当前页**还是保留根任务B**，保存当前页（A+B=12条）
4. 新开一页，下一个根任务（C）从新页开始

**最终分页结果**：
- 第一页：根任务A + 根任务B（12条，虽然超过10条，但为了保证父任务和子任务的完整性）
- 第二页：根任务C（3条）
