...
```vue
<!-- 日报生成表单 -->
<el-form :model="dailyForm" label-width="120px">
  <el-form-item label="报告日期">
    <el-date-picker v-model="dailyForm.reportDate" type="date" value-format="YYYY-MM-DD" />
  </el-form-item>
  ...
  <el-form-item>
    <el-button type="primary" @click="generateDailyReport">生成日报</el-button>
  </el-form-item>
</el-form>
```
...
const generateDailyReport = async () => {
  ...
  try {
    ...
    ElMessage.success('日报生成中，请稍后在历史报告中查看')
...
public class ReportDataAggregationServiceImpl implements ReportDataAggregationService {
    ...
    @Override
    public String aggregateDailyReportData(LocalDate reportDate, Long projectId) {
        // 查询指定日期的任务
        LambdaQueryWrapper<PmTask> wrapper = new LambdaQueryWrapper<>();
        if (projectId != null) {
            wrapper.eq(PmTask::getProjectId, projectId);
        }
        wrapper.ge(PmTask::getStartDate, reportDate.minusDays(7))
               .le(PmTask::getEndDate, reportDate.plusDays(7));
        
        List<PmTask> tasks = pmTaskService.list(wrapper);
        
        // 区分重点任务和非重点任务
        List<PmTask> focusTasks = tasks.stream()
                .filter(t -> t.getIsFocus() != null && t.getIsFocus() == 1)
                .collect(Collectors.toList());
        List<PmTask> normalTasks = tasks.stream()
                .filter(t -> t.getIsFocus() == null || t.getIsFocus() != 1)
                .collect(Collectors.toList());
        
        StringBuilder sb = new StringBuilder();
        sb.append("报告日期：").append(reportDate).append("\n\n");
        
        // 重点任务
        if (!focusTasks.isEmpty()) {
            sb.append("===== 重点任务 =====\n");
            for (PmTask task : focusTasks) {
                appendTaskInfo(sb, task);
            }
        }
    ...
    private void appendTaskInfo(StringBuilder sb, PmTask task) {
        ...
        // 查询任务进展
        List<PmTaskProgressUpdate> updates = progressUpdateService.getByTaskIdAndDate(task.getId(), task.getStartDate(), task.getEndDate());
        if (!updates.isEmpty()) {
            sb.append("进展记录：\n");
