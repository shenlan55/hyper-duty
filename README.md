...
- 甘特图：可视化项目进度和任务时间线
- **AI报告**：基于AI的智能日报周报生成，支持七大维度分类
  - 支持GOC调度、架构治理、入网管理、变更管控、故障治理、运营治理、数智化研运
  - 支持重点任务和普通任务分类展示
  - 支持项目筛选和日期范围选择
  - 异步生成报告，避免超时问题
...
1. **初始化数据库**：
   - 执行 `src/main/resources/sql/hyper_duty_ddl.sql` 创建数据库和表
   - 执行 `src/main/resources/sql/hyper_duty_dml.sql` 初始化数据
   - （可选）手动在系统中添加AI报告菜单，菜单配置参考：
     - 菜单名称：AI报告
     - 菜单路径：/project/ai-report
     - 菜单组件：views/project/AiReport.vue
     - 菜单权限：project:aiReport:list, project:aiReport:generate
2. **启动后端**：
   ```
   mvn spring-boot:run
   # 或
...
#### 模块说明
- **common**：通用工具、配置、切面、异常处理、安全配置等
- **system**：系统管理（员工、部门、角色、菜单、字典、定时任务等）
- **duty**：值班管理（排班、值班记录、请假调班、节假日等）
- **pm**：项目管理（项目、任务、进度、评论、自定义表格等）
- **ai**：AI报告（报告生成、配置、历史记录等）
...
├── src/
│   ├── api/             # API请求
│   ├── components/      # 公共组件（含移动端组件）
│   │   ├── Layout.vue           # 双模布局主组件
│   │   ├── MobileBottomNav.vue  # 移动端底部Tab导航
│   │   ├── MobileDrawer.vue     # 移动端汉堡菜单抽屉
│   │   ├── MobileGuard.vue      # 移动端B类页面访问拦截
│   │   └── BaseTable.vue        # 统一表格组件（PC表格 / 移动端卡片）
│   ├── hooks/           # 组合式函数
│   │   └── useResponsive.js     # 设备检测Hook
│   ├── styles/          # 样式
│   │   ├── mobile.scss          # 移动端全局样式
│   │   └── breakpoints.scss     # 响应式断点定义
│   ├── layout/          # 布局组件
│   ├── router/          # 路由配置
│   ├── store/           # 状态管理
│   ├── utils/           # 工具类
...
- **AI报告配置表（ai_report_config）**：存储AI报告生成配置
- **AI报告生成记录表（ai_report）**：存储AI生成的报告记录
