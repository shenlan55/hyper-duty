...
| AiReportConfig | 实体类 | AI报告配置实体 | src/main/java/com/lasu/hyperduty/entity/AiReportConfig.java |
...
| AiReportServiceImpl | 服务实现 | AI报告服务实现 | src/main/java/com/lasu/hyperduty/service/impl/AiReportServiceImpl.java |
| ReportDataAggregationService | 服务接口 | 报告数据聚合服务 | src/main/java/com/lasu/hyperduty/service/ReportDataAggregationService.java |
| ReportDataAggregationServiceImpl | 服务实现 | 报告数据聚合服务实现 | src/main/java/com/lasu/hyperduty/service/impl/ReportDataAggregationServiceImpl.java |
...
1. **数据聚合**：根据用户选择的日期范围和项目，从数据库中聚合任务数据，区分重点任务和非重点任务
2. **AI生成**：将聚合的数据和提示词模板发送给智谱AI GLM-4-Flash模型，生成结构化报告
3. **异步处理**：使用CompletableFuture异步执行报告生成，避免前端超时
4. **报告存储**：生成的报告存储到ai_report表中，包含报告内容、状态、创建时间等信息
5. **历史查询**：用户可以查看历史生成的报告，支持按报告类型筛选

### 1.7.3 关键功能特性

- **七大维度分类**：支持GOC调度、架构治理、入网管理、变更管控、故障治理、运营治理、数智化研运七大维度
