...
### v2.6 - 2026-05-26
- **影子任务分页逻辑修复**：修复了影子任务的分页逻辑，确保父任务的所有子任务都在同一页显示
  - **核心改进**：不再简单地按 pageSize 切分扁平列表，而是改为按根任务分组
  - **规则1**：如果根任务有子任务，该根任务及其所有子任务必须完整显示在同一页，不被分页截断
  - **规则2**：只有当所有任务都是根任务（没有子任务）时，才按 pageSize（如10条）严格分页
  - **规则3**：判断逻辑：检查当前页是否已有带子任务的根任务，或当前要添加的根任务是否有子任务
  - **实现位置**：仅修改了 `PmTaskShadowServiceImpl.java`，真实任务保持不变
  - **排序逻辑保留**：原有的置顶、状态、优先级、创建时间排序逻辑完全保留
